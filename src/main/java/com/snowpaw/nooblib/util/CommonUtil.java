package com.snowpaw.nooblib.util;

import java.util.ArrayList;

import com.snowpaw.nooblib.device.EnergyDevice;
import com.snowpaw.nooblib.device.EnergyNode;
...
public class CommonUtil {
    ...
    public static void writePulseDevicesArrayToNBT(NBTTagCompound tagCompound, String name, ArrayList<EnergyDevice> devices) {
        int devicesX[];
        int devicesY[];
        int devicesZ[];
        ...
        if (devices != null) {
            ...
            int i = 0;
            for (EnergyDevice entry : devices) {
                devicesX[i] = entry.x;
                devicesY[i] = entry.y;
                devicesZ[i] = entry.z;
                i++;
    ...
    public static ArrayList<EnergyDevice> readPulseDevicesArrayFromNBT(NBTTagCompound tagCompound, String name) {
        ArrayList<EnergyDevice> devices = new ArrayList<EnergyDevice>();
        int devicesX[];
        int devicesY[];
        int devicesZ[];
        ...
        for (int i = 0; i < devicesX.length; i++)
            devices.add(new EnergyDevice(devicesX[i], devicesY[i], devicesZ[i], null));

        return devices;
    }

    public static void writePulseDeviceToNBT(NBTTagCompound tagCompound, String name, EnergyDevice device) {
        if (device != null) {
    ...
    public static EnergyDevice readPulseDeviceFromNBT(NBTTagCompound tagCompound, String name) {
    ...
    public static void writePulsePylonsArrayToNBT(NBTTagCompound tagCompound, String name, ArrayList<EnergyNode> pylons) {
        int nodesX[];
        int nodesY[];
        int nodesZ[];
        int nodesMaster[];
    ...
    public static ArrayList<EnergyNode> readPulsePylonsArrayFromNBT(NBTTagCompound tagCompound, String name) {
        ArrayList<EnergyNode> pylons = new ArrayList<EnergyNode>();
        int nodesX[];
        int nodesY[];
        int nodesZ[];
        int nodesMaster[];
        ...
        for (int i = 0; i < nodesX.length; i++)
            pylons.add(new EnergyNode(nodesX[i], nodesY[i], nodesZ[i], nodesMaster[i]));
    ...
    public static void addChatProbeConnectedMachines(EntityPlayer player, ArrayList<EnergyDevice> machines, World world, int x, int y, int z) {
        if (machines != null && machines.size() != 0) {
            if (!(machines.size() == 1 && machines.get(0).x == x && machines.get(0).y == y && machines.get(0).z == z)) {
                for (EnergyDevice entry : machines)
    ...
    public static void addChatProbeConnectedNodes(EntityPlayer player, ArrayList<EnergyNode> nodes, World world, int x, int y, int z) {
        if (nodes != null && nodes.size() != 0) {
            if (!(nodes.size() == 1 && nodes.get(0).x == x && nodes.get(0).y == y && nodes.get(0).z == z)) {
                for (EnergyNode entry : nodes)
                    if (!(entry.x == x && entry.y == y && entry.z == z))
                        player.addChatMessage(new ChatComponentTranslation("msg.probeConnectedEntry.txt", entry.x, entry.y, entry.z,
                                world.getBlock(entry.x, entry.y, entry.z).getLocalizedName()));
            }
